...
class VoronezhBotCommand  extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName('bd:voronezh:order:create')
            ->setDescription('Creates a voronezh fake order')
            ->addOption(
                'random-mode',
                'r',
            ...
            ->setHelp(
                <<<EOT
                    The <info>%command.name%</info> command creates a voronezh fake order.
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $token = $this->getContainer()->getParameter('voronezh_token');
        $names = [
            'Цветы',
            'Цветы',
            'Цветы',
        ...
        $pickupAddresses = [
            'г. Воронеж, проспект Труда, д. 87',
            'ул. Пушкинская, 5, Воронеж',
            'пр. Революции, 34А, Воронеж'
        ];
        $deliveryAddresses = [
            'ул. Кропоткина, 10А, Воронеж',
            'ул. Пешестрелецкая, 110Б, Воронеж',
            'ул. Кропоткина, 10А, Воронеж',
            'ул. Платонова, 4, Воронеж',
            'Транспортная ул., 12, Воронеж',
            'ул. 25 Января, 4, Воронеж',
            'ул. Владимира Невского, 29, Воронеж',
            'ул. Хользунова, 121-Г, Воронеж',
            'Плехановская ул., 22, Воронеж',
            'Транспортная ул., 12, Воронеж',
            'ул. 25 Января, 4, Воронеж',
            'Никитинская ул., 8А, Воронеж',
            'ул. Пятницкого, 65А, Воронеж'
        ];
        $minutes = ['00', '15', '30', '45'];
